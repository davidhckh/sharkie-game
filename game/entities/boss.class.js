...
export default class Boss extends MovableObject {
  ...
  attack() {
    if (
      ...
      if (this.drawReverse) {
        gsap.to(this, { x: this.x + 400, delay: 0.25, duration: 0.5 });
      } else {
        gsap.to(this, { x: this.x - 400, delay: 0.25, duration: 0.5 });
      }
  ...
  moveTowardsCharacter() {
    if (
      this.game.world.level.character.y +
        this.game.world.level.character.height / 2 <
      1080 / 2
    ) {
      gsap.to(this, { y: -400, delay: 0.25, duration: 0.5 });
      gsap.to(this, { y: -200, delay: 0.75, duration: 0.5 });
    } else {
      gsap.to(this, { y: 100, delay: 0.25, duration: 0.5 });
      gsap.to(this, { y: -200, delay: 0.75, duration: 0.5 });
    }
  }
  ...
  updateIntro() {
    if (
      this.game.world.level.character.x >= this.x - 900 &&
      !this.isIntroducing
    ) {
      this.introduce();
    }
  ...
  changeMovementDirection() {
    if (
      this.x < 200 ||
      this.x + this.width / 2 - this.hitboxRight <
        this.game.world.level.character.x
    ) {
      this.drawReverse = true;
    } else if (
      this.x >= this.introAtX ||
      this.x + this.hitboxRight + this.width / 2 >
        this.game.world.level.character.x
    ) {
