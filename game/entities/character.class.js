...
export default class Character extends MovableObject {
  ...
  checkCollisionsWith(object) {
    if (this.isCollidingWith(object) && !this.freeze) {
      ...
      if (object.name == "jellyfish" && !object.isDead) {
        ...
        (object.name == "coin" || object.name == "poison") &&
        !object.isCollected
      ) {
        object.collect();
        /**boss */
  ...
  shootBubble() {
    if (!this.isShooting && !this.isHitting && !this.isInvincible) {
      ...
      setTimeout(() => {
        ...
        this.game.world.bubbles.push(
          new Bubble(this.game.world.bubbles.length)
        );
      }, 600);
    }
  }
  ...
  updateSwim() {
    ...
    /**update sharkie position and camera target  if possible*/
    if (
      this.right &&
      !this.barrierRight &&
  ...
  checkBarrierCollisions() {
    ...
    this.game.world.level.barriers.forEach((barrier) => {
      if (this.isCollidingWith(barrier)) {
        if (
          this.x + this.width - this.hitboxRight <=
          barrier.x + this.speed + barrier.hitboxLeft
        ) {
          this.barrierRight = true;
        } else if (
          this.x + this.hitboxLeft >=
          barrier.x + barrier.width - barrier.hitboxRight - this.speed
        ) {
          this.barrierLeft = true;
        } else if (
          this.y + this.height - this.hitboxBottom <=
          barrier.y + barrier.hitboxTop + this.speed
        ) {
          this.barrierBottom = true;
        } else if (
          this.y + this.hitboxTop <=
          barrier.y + barrier.height - barrier.hitboxBottom
        ) {
